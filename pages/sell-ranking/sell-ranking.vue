...
<template>
  <view class="sell-ranking">
    ...
    <view class="top-area">
      ...
      <view
        v-if="tabCurTwo === 0 && tabCurOne !== 3"
        class="top-area-bg single-bg"
      >
        <image
          src="../../static/selle_ranking/single_bg.png"
          mode="scaleToFill"
        />
      </view>
      <view
        v-if="tabCurTwo !== 0 && tabCurOne !== 3"
        class="top-area-bg shop-bg"
      >
        <image
          src="../../static/selle_ranking/shop_bg.png"
          mode="scaleToFill"
        />
      </view>
      <!-- 销售排行tab -->
      <view class="top-area-tabs" :class="tabCurOne === 3 ? 'chart' : ''">
        <view
          v-for="(item, i) of tabsOne"
          :key="i"
          class="tabs-item"
          :class="tabCurOne === i ? 'active' : ''"
      ...
      <!-- 线上/线下订单 tab -->
     <!-- <view
        v-if="tabCurTwo === null && tabCurOne !== 3"
        class="top-area-tabs order"
        :class="tabCurOne === 3 ? 'chart' : ''"
      >
        <view
          v-for="(item, i) of orderTab"
          :key="i"
          class="tabs-item"
          :class="orderTabCur === i ? 'active' : ''"
          @click="chooseOrderTab(i)"
        >
          <text>{{ item.title }}</text>
        </view>
      </view> -->
      ...
      >
        <view
          v-for="(item, i) of tabsTwo"
          :key="i"
          class="tabs-item"
          :class="tabCurTwo === i ? 'active' : ''"
          @click="chooseTabTwo(i, item)"
        >
          <text>{{ item.title }}</text>
        </view>
      </view>
      ...
      <view v-if="tabCurOne !== 3" class="time-select">
        <picker
          ...
          :value="reqForm.star_time"
          :end="reqForm.end_time"
          mode="date"
        >
      ...
      <!-- 结束时间选择 -->
      <view
        v-if="tabCurTwo === null && tabCurOne !== 3 && pageIndex === 0"
        class="search-area"
      >
        <view class="search-box">
          <view class="icon">
            <image
              src="../../static/selle_ranking/search.png"
              mode="scaleToFill"
            />
          </view>
          <input
      ...
      <!-- 销售额排行 数据卡片 -->
      <view
        v-if="saleData && tabCurTwo === null && tabCurOne !== 3"
        class="data-card"
      >
        <view class="data-card-item">
          <text class="title">总销售额</text>
          <text class="num">{{ saleData.data.total_sale_money }}</text>
        </view>
        <view class="data-card-item">
          <text class="title">总销售数量</text>
          <text class="num">{{ saleData.data.total_sale_num }}</text>
        </view>
        <view class="data-card-item">
          <text class="title">总销售单数</text>
          <text class="num">{{ saleData.data.total_sale_count }}</text>
        </view>
          ...
          <view class="info-bottom">
            <view class="star-box">
              ...
              <text class="star-box-num">
                <text
                  v-if="tabCurOne === 2 && item.sale_money_levelNum !== 0"
                  class="change"
                  :class="item.sale_money_level === 'up' ? 'up' : ''"
                >
            ...
            <view class="star-box">
              ...
              <text class="star-box-num">
                <text
                  v-if="tabCurOne === 2 && item.sale_num_levelNum !== 0"
                  class="change"
                  :class="item.sale_num_level === 'up' ? 'up' : ''"
                >
                  {{ item.sale_num_level === 'up' ? '+' : '-'
                  }}{{ item.sale_num_levelNum
                  }}{{ item.sale_num_level === 'up' ? '↑' : '↓' }}
                </text>
                {{ item.sale_num }}
              </text>
            ...
            <view class="star-box">
              ...
              <text class="star-box-num">
                <text
                  v-if="tabCurOne === 2 && item.sale_count_levelNum !== 0"
                  class="change"
                  :class="item.sale_count_level === 'up' ? 'up' : ''"
                >
                  {{ item.sale_count_level === 'up' ? '+' : '-'
                  }}{{ item.sale_count_levelNum
                  }}{{ item.sale_count_level === 'up' ? '↑' : '↓' }}
                </text>
                {{ item.sale_count }}
              </text>
        ...
        <view v-for="(item, i) of listData" :key="i" class="single-list-item">
          <view class="item-left">
            ...
            <view class="icon">
              <image
                v-if="i === 0"
                src="../../static/selle_ranking/first.png"
                mode="scaleToFill"
              />
              <image
                v-else-if="i === 1"
                src="../../static/selle_ranking/second.png"
                mode="scaleToFill"
              />
              <image
                v-else-if="i === 2"
                src="../../static/selle_ranking/third.png"
                mode="scaleToFill"
              />
            </view>
            <view class="avatar">
        ...
        <view class="shop-data">
          <view class="shop-data-left">
            ...
            <view class="shop-avatar">
              <image
                v-if="imData.im_data.store_img"
                :src="imData.im_data.store_img"
                mode="scaleToFill"
              />
              <image v-else src="../../static/user.png" mode="scaleToFill" />
            </view>
        ...
        <view class="team-data">
          <view class="team-data-left">
            ...
            <view class="team-avatar">
              <image
                v-if="imData.im_data.brand_img"
                :src="imData.im_data.brand_img"
                mode="scaleToFill"
              />
              <image v-else src="../../static/user.png" mode="scaleToFill" />
            </view>
...
export default {
  ...
  onLoad(options) {
	  if (options.type) {
	  	this.reqForm.type = options.type
	  }
    /* 如果是跳转的页面,则设置好传过来的参数 */
    if (options.data) {
  ...
  methods: {
    ...
    getSaleList() {
      ...
      data.start_date = this.reqForm.star_time
      data.end_date = this.reqForm.end_time
    ...
    chooseTabOne(e) {
      ...
      // this.saleForm.page = 1
      this.reqForm.page = 1
	  this.reqForm.sorts=this.tabCurTwo+1
      this.listData = []
