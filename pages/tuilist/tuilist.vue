	...
	<view>
		...
		<view class="bg">
			<view class="search-box">
				...
				<text class="search-btn" @click="getdata">搜索</text>
			</view>
		</view>
		<view class="top">
			<view class="top_left">
		...
		<view class="box" v-if="datalist.length > 0">
			<view class="item" v-for="item,index in datalist" :key='index'>
				...
				<view class="item_right" @click="tomytwitter(item.id)">
					...
					<image src="../../static/index/next@2x(1).png" class="right_icon" mode="widthFix"></image>
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods:{
			tonext(id){
				this.$Common.commonJump('pages/tuilisttwo/tuilisttwo?id='+id)
			},
			tomytwitter(id){
				this.$Common.commonJump('pages/mytwitter/mytwitter?zhan_id='+id)
			},
			getdata(){
				this.$api.getZhanTweetList({name:this.search},{
					success:(ret)=>{
						if(ret.code == 200){
