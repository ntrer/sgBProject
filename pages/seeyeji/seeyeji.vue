    ...
    <view v-for="(item, i) of cardData" :key="i" class="card-wrap">
      ...
      >
        ...
        <view class="card-item-info">
          ...
          <text class="person">{{ pageData[card.name]?pageData[card.name]:0 }} 人</text>
        </view>
      </view>
    </view>
  ...
  methods: {
    ...
    navTo(item) {
      ...
      switch (type) {
        ...
        //   this.$Common.commonJump(path + '?type=' + type+'&ii=true')
        //   break
        /* 跳转 签单 页面 */
        case 'off_line_order':
          path = '/pages/results-order/results-order'
          type = '1'
          this.$Common.commonJump(path + '?type=' + type+'&ii=true&id=' + this.idd)
          break
        /* 跳转 签单 页面 */
