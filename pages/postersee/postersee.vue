...
export default {
	...
	onLoad(params) {
		...
		// this.$api.getCarryQrcode({
		// 	id: this.thisid
		// }, {
		// 	success: (ret) => {
		// 		if (ret.code == 200) {
		// 			this.posterinfo = ret.data
		// 			this.address = ret.data.url + '?url_path=/engineer/bind/phone&&carry_code=' + ret.data.carry_code + '&&act_id=' + uni.getStorageSync('active_id')+ '&&carry_user_code=' + ret.data.carry_code;
		// 			this.pic = ret.data.imgs
		// 		}
		// 	}
		// })
	},
			...
			},
			{
				success: (ret) => {
					if (ret.code == 200) {
						...
						this.address = ret.data.url + '?url_path=/engineer/bind/phone&&carry_code=' + ret.data.carry_code + '&&act_id=' + uni.getStorageSync('active_id').data + '&&carry_user_code=' + ret.data.carry_code;
		...
		onCreatImg() {
			...
			query
				...
				.boundingClientRect((data) => {
					...
					ctx.strokeRect(115.5, 205, 65, 65); //绘制边框


					ctx.drawImage(that.src, 121, 210, 55, 55); //填充进二维码
					...
					ctx.fillText('内购个人专属编号:' + this.posterinfo.carry_code, 47.5, 187.5);

					ctx.setFontSize(16);
					ctx.setTextAlign('left');
					ctx.font = 'normal bold 7px sans-serif'; //设置对于坐标点的对齐方式
					ctx.setFillStyle('#333333'); //设置内容2的文字样式
					ctx.fillText('内购优惠截止日期:', 15, 205);
					ctx.fillText(this.posterinfo.end_time, 15, 216);

					ctx.fillText('内购开展认证门店:', 15, 231);
					ctx.fillText(this.posterinfo.store_name, 15, 240);

					ctx.fillText('内购客服专线:', 15, 255);
					ctx.fillText(this.posterinfo.store_phone, 15, 270);

					ctx.fillText('内购专属门店地址:', 15, 285);
					ctx.fillText(this.posterinfo.store_address, 15, 300);
