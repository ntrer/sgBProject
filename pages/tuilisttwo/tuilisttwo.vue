...
<script>
	export default {
		...
		methods:{
			grtdata(){
				this.$api.getBrandTweetList({name:this.search,id:this.idd},{
					success:(ret)=>{
						if(ret.code == 200){
			...
			tomytwitter(id){
				this.$Common.commonJump('pages/mytwitter/mytwitter?brand_id='+id)
			},
			tonext(id){
				this.$Common.commonJump('pages/tuilistthree/tuilistthree?id='+id)
			}
		}
	}
</script>
